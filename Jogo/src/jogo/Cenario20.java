...
public class Cenario20 extends Cenario{
	...
	public Cenario20 (Window window, Jogador player) {
		
		janela = window;
		cena = new Scene();
		cena.loadFromFile(URL.scenario("Cenario20.scn"));
		jogador = player;
		teclado = janela.getKeyboard();
		troll = new Troll[2];
		chave = new Key(380, 160);
...
//			cena.draw();
			...
			jogador.vida(janela);
			chave.x += cena.getXOffset();
			chave.y += cena.getYOffset();
			jogador.x += cena.getXOffset();
			jogador.y += cena.getYOffset();
			jogador.draw();
			chave.draw();
			chave.PickUp(jogador);
