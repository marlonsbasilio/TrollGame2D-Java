package jogo;

import jplay.Keyboard;
import jplay.Scene;
import jplay.URL;
import jplay.Window;


public class Cenario12 extends Cenario{
	
	private Window janela;
	private Scene cena;
	private Jogador jogador = null;
	private Keyboard teclado;
	private Troll troll[];
	private boolean access = false;
	private Ouro ouro;
	private Diamante diamante;
	
	public Cenario12 (Window window, Jogador player) {
		
		janela = window;
		cena = new Scene();
		cena.loadFromFile(URL.scenario("Cenario12.scn"));
		jogador = player;
		teclado = janela.getKeyboard();
		troll = new Troll[4];
		diamante = new Diamante(480, 140);
		ouro = new Ouro(350,350);
	}
	public void run() {
		
		if (access == false) {
			for (int i = 0; i < troll.length; i++) {
				troll[i] = new Troll(300 + (1 + (i*50)), 200 + (1 + (i*20)));
			}
			access = true;
		}
		
		while(true) {
			jogador.controle(janela, teclado);
			jogador.caminho(cena);
			cena.moveScene(jogador);
			jogador.vida(janela);
			jogador.x += cena.getXOffset();
			jogador.y += cena.getYOffset();
			jogador.draw();
			jogador.potion_use(teclado);
			
			ouro.x += cena.getXOffset();
			ouro.y += cena.getYOffset();
			ouro.draw();
			diamante.x += cena.getXOffset();
			diamante.y += cena.getYOffset();
			diamante.draw();
			
			int cont = 0;

			for (int i = 0; i < troll.length; i++) {
				if (troll[i].vida <= 0) {
					cont++;
				}
			}
			if (cont == troll.length) {
			ouro.Steal(jogador);
			diamante.Steal(jogador);
			}
			
			for (int i = 0; i < troll.length; i++) {
			troll[i].atacar(jogador);
			troll[i].caminho(cena);
			troll[i].perseguir(jogador.x, jogador.y);
			troll[i].x += cena.getXOffset();
			troll[i].y += cena.getYOffset();
			jogador.atirar(janela, cena, teclado, troll[i]);
			troll[i].morrer();
			troll[i].draw();			
			}
			janela.update();
			mudarCenario();
		}
	}
	private void mudarCenario() {
		if (tileCollision(2, jogador, cena) == true) {
			jogador.x = 380;
			jogador.y = 240;
			jogador.id_sala = 11;
			jogador.c11.run();
		}
		if (tileCollision(3, jogador, cena) == true) {
			jogador.x = 300;
			jogador.y = 240;
			jogador.id_sala = 13;
			jogador.c13.run();
		}
		if (tileCollision(4, jogador, cena) == true) {
			jogador.x = 380;
			jogador.y = 160;
			jogador.id_sala = 7;
			jogador.c7.run();
		}
	}
}
